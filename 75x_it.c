...
u8  temp;
/* Extern variables ----------------------------------------------------------*/
volatile u32 GlobalTick;

//extern volatile u8 canstatus;
// error states
extern volatile bool bBusOff;
extern volatile bool bBusWarn;
extern volatile bool bErrorPassive;
...
//  u8  chksum = 0;
  ...
  {        
   USB2CAN_next_status = CAN->SR; 
  }
  else if(CAN->IDR >= 1 && CAN->IDR <= 28)
  { 
    while (CAN_ReceiveMessage(msgobj, FALSE, &CanMsg) !=  SUCCESS);           
                               
        CanMsg.IdType &= 0x00FF;   // data frame
        CanMsg.Timestamp = GlobalTick;
                      
      if( CanListPut(&CanRXlist,&CanMsg) )            
      {
        Statistics.ReceivedFrames++;                
        Statistics.ReceivedDataBytes += CanMsg.Dlc;
      }
      else 
      {        
        Statistics.Overruns++;                 
