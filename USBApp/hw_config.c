...
/* MRCC configuration --------------------------------------------------------*/
  ...
  OSC4MStartUpStatus = MRCC_WaitForOSC4MStartUp();
    ...
    // Set CKTIM (Configures the TIM clock) 32MHZ
    MRCC_CKTIMConfig( MRCC_HCLK_Div1 );  // Div2
  
    // Set PCLK (Configures the APB clock) 32MHz
...
/************** STD TX Objects *******************/
   ...
   while( CAN_SetTxMsgObj( 30 , CAN_STD_ID ) != SUCCESS);
  
/************** EXT TX Objects *******************/
  
  while( CAN_SetTxMsgObj( 31 , CAN_EXT_ID ) != SUCCESS);  
...
/************** STD RX Objects *******************/
  ...
  {
    while( CAN_SetRxMsgObj( temp, CAN_STD_ID, 0, CAN_LAST_STD_ID, TRUE ) != SUCCESS );
  }  
...
/************** EXT RX Objects ******************/
  ...
  {
   while( CAN_SetRxMsgObj( temp, CAN_EXT_ID, 0, CAN_LAST_EXT_ID, TRUE ) != SUCCESS );
  }       
...
void Set_TB(void)
{
 /* TB configuration in Timing mode ------------------------------------------*/
  ...
  TB_InitStructure.TB_Prescaler = 126; //252;  //was63; // 64-1 //239; /* TB clock = 60MHz / (239+1) = 250KHz*/
