...
/* Private variables ---------------------------------------------------------*/
/* array of pre-defined timing parameters for standard bitrates*/ // ABP = 8MHz // GS
...
/* array of pre-defined timing parameters for standard bitrates*/ // ABP = 32MHz // GS
u16 CanTimings[] = {
  //  tseg1 ; tseg 2 ; sjw ; brp
  CAN_TIMING(13, 2, 1, 200), //   10 kbit/s   87.5%
  CAN_TIMING(13, 2, 1, 100), //   20 kbit/s   87.5%
  CAN_TIMING(13, 2, 1, 40),  //   50 kbit/s   87.5%
  CAN_TIMING(13, 2, 1, 20),  //  100 kbit/s   87.5%
  CAN_TIMING(13, 2, 1, 16),  //  125 kbit/s   87.5%
  CAN_TIMING(13, 2, 1, 8),   //  250 kbit/s   87.5%
  CAN_TIMING(13, 2, 1, 4),   //  500 kbit/s   87.5%
  CAN_TIMING( 6, 1, 1, 5),   //  800 kbit/s   87.5%
  CAN_TIMING(13, 2, 1, 2),   //  1 Mbit/s     87.5%
}; 
...
ErrorStatus CAN_SetTxMsgObj(u32 msgobj, u32 idType)
{
  ...
  {
    //CAN->sMsgObj[msg_if].M2R = CAN_M2R_MDIR; // GS
    CAN->sMsgObj[msg_if].M2R = 0; // GS 
    CAN->sMsgObj[msg_if].A2R = CAN_A2R_MSGVAL | CAN_A2R_DIR;
  }
  else
  {
    //CAN->sMsgObj[msg_if].M2R = CAN_M2R_MDIR | CAN_M2R_MXTD; // GS
    CAN->sMsgObj[msg_if].M2R = 0; // GS 
...
ErrorStatus CAN_SetRxMsgObj(u32 msgobj, u32 idType, u32 idLow, u32 idHigh, bool singleOrFifoLast)
{
  ...
  if (idType == CAN_STD_ID)
  {
    ...
    CAN->sMsgObj[msg_if].M2R = CAN_M2R_MXTD | STD_RANGE_ID_MSK(idLow, idHigh); // GS
...
ErrorStatus CAN_SendMessage(u32 msgobj, canmsg* pCanMsg)
{
  ...
  if( !(pCanMsg->IdType & CAN_RTR_ID )) // GS    
...
ErrorStatus CAN_ReceiveMessage(u32 msgobj, bool release, canmsg* pCanMsg)
{
  ...
  else
  {
    ...
    pCanMsg->Id = ( CAN->sMsgObj[1].A1R ) | (( CAN->sMsgObj[1].A2R & 0x1fff ) << 16); // GS corrected AKHE's bug
  }
  
  if ((CAN->sMsgObj[1].A2R & CAN_A2R_DIR )) // GS  added RTR
    pCanMsg->IdType |= CAN_RTR_ID;  
  else
    pCanMsg->IdType &= ~CAN_RTR_ID; 
