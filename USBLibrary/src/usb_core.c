...
void DataStageIn(void) 
{
	...
	DataBuffer = (*pEPinfo->CopyData)(Length);  // GS cia grazina pointeri nuo kurios vietos kopijuoti
        
        UserToPMABufferCopy(DataBuffer,GetEPTxAddr(ENDP0),Length);

	SetEPTxCount(ENDP0, Length);

	pEPinfo->Usb_wLength -= Length;  // GS slenkam per struktura kol visa issiusim
...
void Data_Setup0(void)
{
	...
	if (CopyRoutine) {
		...
		(*CopyRoutine)(0);     // GS cia uzpildo Ctrl_Info->Usb_wLength; pagal kintamojo arba strukturos dydi
		Result = USB_SUCCESS;
	}
       else {
		Result = (*pProperty->Class_Data_Setup)(pInformation->USBbRequest); // GS jei nerado is standartiniu klasiu ziurim konkretiem atvejam
		if (Result == USB_NOT_READY) {
	...
	if (Result == USB_UNSUPPORT || pInformation->Ctrl_Info.Usb_wLength == 0) {
        ...
        /* GS pasitikrina kuria kryptimi bus siunciami duomenys pagal bmReqestType 7 bita */
	if (ValBit(pInformation->USBbmRequestType, 7)) {
		/* Device ==> Host */
		u32	wLength = pInformation->USBwLength;

		/* Restrict the data length to be the one host asks */
		if (pInformation->Ctrl_Info.Usb_wLength > wLength)      // GS grazina tik toki arba ne didesni strukturos ilgi koks buvo uzklaustas
			pInformation->Ctrl_Info.Usb_wLength = wLength;

		pInformation->Ctrl_Info.PacketSize = pProperty->MaxPacketSize;
		DataStageIn(); /* GS Data stage of a Control Read Transfer */
	}
	else { /* Host ==> Device */
		pInformation->ControlState = OUT_DATA;
...
u8 In0_Process(void)
{
	...
	else if (ControlState == WAIT_STATUS_IN) // GS po duomenu gavimo is PC (HOST==>Controller) laukiama 0 DATA ilgio patvirtinimo ir jis jau katik gautas
	{                                        // t.y. kai gauti visi duomenys is PC status pakeiciamas i WAIT_STATUS_IN ir laikiama 0 DATA
...
u8 Out0_Process(void)
{
	...
	if(ControlState == OUT_DATA || ControlState == LAST_OUT_DATA)
        {
	  DataStageOut();  // kopijuoja i user buferi ir kai viskas priimta WAIT_STATUS_IN
          ControlState = pInformation->ControlState; // <--- reikejo sito nes ERROR ??? (In0_Process yra sita eilute)
        } 
