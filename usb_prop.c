...
void Virtual_Com_Port_Reset(void)
{
  ...
  SetEPTxAddr(ENDP1, ENDP1_TXADDR);  // GS 0xC0 192
  SetEPTxStatus(ENDP1, EP_TX_NAK);   // buff dydis 64 baitai
  SetEPRxStatus(ENDP1, EP_RX_DIS);
  
  /* Initialize Endpoint 2 */
  SetEPType(ENDP2, EP_INTERRUPT);
  SetEPTxAddr(ENDP2, ENDP2_TXADDR);  //GS 0x100 256 (ilgis 16)
  SetEPRxStatus(ENDP2, EP_RX_DIS);
  SetEPTxStatus(ENDP2, EP_TX_NAK);
  
   /* Initialize Endpoint 3 */
  SetEPType(ENDP3, EP_BULK);
  SetEPRxAddr(ENDP3, ENDP3_RXADDR);  //GS 0x110 272
  SetEPRxCount(ENDP3, VIRTUAL_COM_PORT_DATA_SIZE); // 64
